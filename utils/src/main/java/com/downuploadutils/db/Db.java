package com.downuploadutils.db;
...
public class Db {
    ...
    public void insertData(DownloadData data) {
        ...
        values.put("child_task_count", data.getChildTaskCount()); //子线程数量
        values.put("current_length", data.getCurrentLength());  //现在的长度
        values.put("total_length", data.getTotalLength()); //全部的长度
        values.put("percentage", data.getPercentage()); //百分比
        values.put("status", data.getStatus()); //地位
        values.put("last_modify", data.getLastModify());    //最后修改
        values.put("date", data.getDate()); //时间
